...
# функция создания таблицы 'alt_users_churn' в БД
def create_table():
    import sqlalchemy
    from sqlalchemy import MetaData, Table, Column, Integer, Float, String, DateTime, UniqueConstraint

    hook = PostgresHook('destination_db')
    db_conn = hook.get_sqlalchemy_engine()

    if not sqlalchemy.inspect(db_conn).has_table('alt_users_churn'):
        metadata = MetaData()
        create_table = Table(
            'alt_users_churn',
            metadata,
...
# функция подключения к БД и извлечения набора данных
def extract(**kwargs):
    ...
    ti = kwargs['ti']
    ti.xcom_push('exctracted_data', data)


# функиця преобразования датасета
...
# функция загрузки датасета в хранилище
def load(**kwargs):
    ti = kwargs['ti']
